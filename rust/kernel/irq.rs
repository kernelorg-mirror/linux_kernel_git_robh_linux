...
impl<T: ForeignOwnable> Drop for InternalRegistration<T> {
    fn drop(&mut self) {
        ...
        //
        // SAFETY: When `new` succeeds, the irq was successfully requested, so it is ok to free
        // it here.
...
///     irq::Registration::new(irq, data, irq::flags::SHARED, fmt!("example_{irq}"))
/// }
/// ```
...
impl<H: Handler> Registration<H> {
    ...
    pub fn new(irq: u32, data: H::Data, flags: usize, name: fmt::Arguments<'_>) -> Result<Self> {
        // SAFETY: `handler` only calls `H::Data::borrow` on `raw_data`.
        Ok(Self(unsafe {
            InternalRegistration::new(irq, Some(Self::handler), None, flags, data, name)?
        }))
    }
