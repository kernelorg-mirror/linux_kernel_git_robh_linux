...
static struct imx_usbmisc_data *usbmisc_get_init_data(struct device *dev)
{
	 ...
	 */
	if (of_property_present(np, "disable-over-current")) {
		data->disable_oc = 1;
	} else if (of_property_present(np, "over-current-active-high")) {
		data->oc_pol_active_low = 0;
		data->oc_pol_configured = 1;
	} else if (of_property_present(np, "over-current-active-low")) {
