...
static int omap_dm_timer_probe(struct platform_device *pdev)
{
	...
	if (dev->of_node) {
		if (of_property_present(dev->of_node, "ti,timer-alwon"))
			timer->capability |= OMAP_TIMER_ALWON;
		if (of_property_present(dev->of_node, "ti,timer-dsp"))
			timer->capability |= OMAP_TIMER_HAS_DSP_IRQ;
		if (of_property_present(dev->of_node, "ti,timer-pwm"))
			timer->capability |= OMAP_TIMER_HAS_PWM;
		if (of_property_present(dev->of_node, "ti,timer-secure"))
