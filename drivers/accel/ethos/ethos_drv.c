...
#define ETHOS_IOCTL(n, func, flags) \
	...
	ETHOS_IOCTL(DEV_QUERY, bo_create, 0),
	ETHOS_IOCTL(DEV_QUERY, bo_mmap_offset, 0),
};
...
static int ethos_reset(struct ethos_device *ethosdev)
{
	...
	writel_relaxed(0x0, ethosdev->regs + NPU_BASEP(0));			// DRAM
	writel_relaxed(ethosdev->sramphys, ethosdev->regs + NPU_BASEP(1));	// SRAM
