...
static bool filter_branch_privilege(struct perf_branch_entry *entry, u64 branch_sample_type)
{
	...
	if (from_user == to_user)
		return ((entry->priv == PERF_BR_PRIV_KERNEL) && !exclude_kernel) ||
			((entry->priv == PERF_BR_PRIV_USER) && (branch_sample_type & PERF_SAMPLE_BRANCH_USER));
...
{
	return test_bit(entry->type, event_type_mask) && filter_branch_privilege(entry, branch_sample);
}
